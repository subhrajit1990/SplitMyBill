...
public class BillManagerService implements IBillManagerService {
	...
	public AddExpensesListResponse fetchExpenses(int groupId, String billId) {
		...
		List<AddExpensesResponse> tempExpeseList = new ArrayList<>();
		Map<String, List<AddExpensesResponse>> addExpenseList = new HashMap<>();
		AddExpensesListResponse addExpensesListResponse = new AddExpensesListResponse();
		Iterable<AddBillDomain> addBillDomain = null;
		if (billId != null) {
		...
		for (AddBillDomain tempAddBillDomain : addBillDomain) {
	...
	public FetchGroupsResponse fetchGroups(FetchGroupsRequest fetchGroupsRequest, String channel,
		...
		List<CreateGroupDomain> fetchGroupsData = null;
		...
		try {
			...
			if (!fetchGroupsData.isEmpty()) {
				...
				for (CreateGroupDomain groupsResults : fetchGroupsData) {
					Groups gp = new Groups();
					gp.setCreatedDt(groupsResults.getCreatedDt());
					gp.setGroupId(groupsResults.getId().toString());
					gp.setGroupName(groupsResults.getGroupName());
					gp.setGroupType(groupsResults.getGroupType());
					groups.add(gp);
				}
